...
async function list(filter) {
  const response = await axios.get(
    `/usermatches?page=${filter.page}&limit=${filter.limit}

    &usermatches=${filter.usermatches ? filter.usermatches : ''}
...
const actions = {
  ...
  doFilter: (request, filter) => async (
    ...
    try {
      ...
      dispatch({
        type: 'USERMATCHES_LIST_FILTERED',
        payload: {
          rows: response.rows,
  ...
  doDelete: (filter, id) => async (dispatch) => {
    try {
      ...
      dispatch({
        type: 'USERMATCHES_LIST_DELETE_ERROR',
      });
    }
  },
