...
import * as dataFormat from 'pages/CRUD/Usersurvey/table/UsersurveyDataFormatters';

import actions from 'actions/usersurvey/usersurveyListActions';
...
const UsersurveyTable = () => {
  ...
  const count = useSelector((store) => store.usersurvey.list.count);
  const modalOpen = useSelector((store) => store.usersurvey.list.modalOpen);
  const rows = useSelector((store) => store.usersurvey.list.rows);
  const idToDelete = useSelector((store) => store.usersurvey.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="usersurvey" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>Usersurvey</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/usersurvey/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/usersurvey/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Usersurvey'
                : 'http://localhost:8080/api-docs/#/Usersurvey'
            }
          >
            API documentation for usersurvey
          </LinkMaterial>
        </div>
      </Widget>
