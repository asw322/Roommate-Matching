...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/usersurvey/autocomplete`, { params });
  return response.data;
}

const UsersurveySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.usersurvey.hasPermissionToCreate,
});

export default connect(select)(UsersurveySelectItem);
