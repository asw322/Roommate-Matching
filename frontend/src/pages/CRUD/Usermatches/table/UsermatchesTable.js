...
import * as dataFormat from 'pages/CRUD/Usermatches/table/UsermatchesDataFormatters';

import actions from 'actions/usermatches/usermatchesListActions';
...
const UsermatchesTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    {label: 'Matched Id', title: 'matchedId'},{label: 'Matched Type', title: 'matchedType'},
  ...
  const count = useSelector((store) => store.usermatches.list.count);
  const modalOpen = useSelector((store) => store.usermatches.list.modalOpen);
  const rows = useSelector((store) => store.usermatches.list.rows);
  const idToDelete = useSelector((store) => store.usermatches.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="usermatches" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>Usermatches</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/usermatches/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/usermatches/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Usermatches'
                : 'http://localhost:8080/api-docs/#/Usermatches'
            }
          >
            API documentation for usermatches
          </LinkMaterial>
        </div>
      </Widget>
