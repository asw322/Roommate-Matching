...
const UserpreferenceTable = () => {
  ...
  const idToDelete = useSelector((store) => store.userpreference.list.idToDelete);
  ...
  const columns = [

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="userpreference" openModal={openModal} {...params} />,
      }
  ];
      ...
      <Widget title={<h4>Userpreference</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/userpreference/${e.id}/edit`)}}
            autoHeight
          />
        </div>
