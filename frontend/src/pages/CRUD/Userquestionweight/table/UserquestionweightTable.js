...
import * as dataFormat from 'pages/CRUD/Userquestionweight/table/UserquestionweightDataFormatters';

import actions from 'actions/userquestionweight/userquestionweightListActions';
...
const UserquestionweightTable = () => {
  ...
  const count = useSelector((store) => store.userquestionweight.list.count);
  const modalOpen = useSelector((store) => store.userquestionweight.list.modalOpen);
  const rows = useSelector((store) => store.userquestionweight.list.rows);
  const idToDelete = useSelector((store) => store.userquestionweight.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="userquestionweight" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>Userquestionweight</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/userquestionweight/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/userquestionweight/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Userquestionweight'
                : 'http://localhost:8080/api-docs/#/Userquestionweight'
            }
          >
            API documentation for userquestionweight
          </LinkMaterial>
        </div>
      </Widget>
...
export default UserquestionweightTable;
