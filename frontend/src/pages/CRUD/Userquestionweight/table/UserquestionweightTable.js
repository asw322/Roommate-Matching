...
const UserquestionweightTable = () => {
  ...
  const modalOpen = useSelector(
    (store) => store.userquestionweight.list.modalOpen,
  );
  const rows = useSelector((store) => store.userquestionweight.list.rows);
  const idToDelete = useSelector(
    (store) => store.userquestionweight.list.idToDelete,
  );
      ...
      <Widget title={<h4>Userquestionweight</h4>} disableWidgetMenu>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/userquestionweight/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
